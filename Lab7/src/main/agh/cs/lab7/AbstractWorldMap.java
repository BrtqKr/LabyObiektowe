...
public abstract class AbstractWorldMap implements IWorldMap, IPositionChangeObserver
{
    ...
    public void run(MoveDirection[] directions)
    {
        for (int i = 0; i < directions.length; i++) {
            ...
            postionChanged(before,actualCar.getPosition());
    ...
    public void postionChanged(Position oldPosition, Position newPosition)
    {
        if(!oldPosition.equals(newPosition))
        {
             IObject car=this.hashObjects.get(oldPosition);
             this.hashObjects.remove(oldPosition);
             this.hashObjects.put(newPosition,car);
        }
